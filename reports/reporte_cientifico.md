...
Generado 2026-09-26 05:39:03 · 7180 sorteos analizados (2025-07-21 a 2026-09-25)
...
- Método de Fisher combinando 4 tests independientes: estadístico=12.9545, p-valor combinado=0.1134.

### 2. ¿Existe evidencia estadística de patrones?
- Chi-cuadrado (uniformidad): NO se rechaza H0 (estadístico=87.9109, p=0.7799). 99 grados de libertad.
- Kolmogorov-Smirnov (uniformidad): Se RECHAZA H0 (estadístico=0.0175, p=0.0237). Aproximación continua sobre datos discretos (orientativa).
- Anderson-Darling (uniformidad): NO se rechaza H0 (estadístico=1.8245, sin p-valor exacto (valor crítico tabulado)). Comparado contra valor crítico tabulado 2.492 (D'Agostino & Stephens 1986).
- Wald-Wolfowitz (rachas): NO se rechaza H0 (estadístico=-0.2360, p=0.8134). 3581 rachas observadas sobre 7180 valores (esperadas ~3591.0).
- Test de independencia (X_t vs X_t-1): NO se rechaza H0 (estadístico=97.5025, p=0.1022). Tabla 10x10, 81 gl, frecuencia esperada mínima 63.1 (válida).
...
- Espectro de Fourier: potencia relativa del pico dominante = 0.0020. Un proceso aleatorio real tiene un espectro aproximadamente plano (sin picos dominantes); este valor bajo no sugiere periodicidad.
...
- Kolmogorov-Smirnov (uniformidad): p=0.0237, umbral BH=0.0125, significativo tras corrección: no.
- Test de independencia (X_t vs X_t-1): p=0.1022, umbral BH=0.0250, significativo tras corrección: no.
- Chi-cuadrado (uniformidad): p=0.7799, umbral BH=0.0375, significativo tras corrección: no.
- Wald-Wolfowitz (rachas): p=0.8134, umbral BH=0.0500, significativo tras corrección: no.
...
- ADVERTENCIA (entropía condicional): Tabla conjunta de 10000 celdas con solo 7179 transiciones observadas: la mayoría de las celdas tiene 0 o 1 observaciones. La entropía condicional empírica está sesgada hacia abajo en este régimen (subestima la incertidumbre real) y NO debe interpretarse como evidencia de dependencia. Usar el test de independencia (chi-cuadrado sobre bins) para esa conclusión, no este número.
