...
Generado 2026-07-31 11:05:02 · 6200 sorteos analizados (2025-07-21 a 2026-07-30)
...
- Método de Fisher combinando 4 tests independientes: estadístico=28.3700, p-valor combinado=0.0004.

### 2. ¿Existe evidencia estadística de patrones?
- Chi-cuadrado (uniformidad): NO se rechaza H0 (estadístico=95.4516, p=0.5823). 99 grados de libertad.
- Kolmogorov-Smirnov (uniformidad): Se RECHAZA H0 (estadístico=0.0174, p=0.0459). Aproximación continua sobre datos discretos (orientativa).
- Anderson-Darling (uniformidad): NO se rechaza H0 (estadístico=1.4428, sin p-valor exacto (valor crítico tabulado)). Comparado contra valor crítico tabulado 2.492 (D'Agostino & Stephens 1986).
- Wald-Wolfowitz (rachas): Se RECHAZA H0 (estadístico=-3.3201, p=0.0009). 2970 rachas observadas sobre 6200 valores (esperadas ~3100.7).
- Test de independencia (X_t vs X_t-1): Se RECHAZA H0 (estadístico=106.8595, p=0.0287). Tabla 10x10, 81 gl, frecuencia esperada mínima 53.3 (válida).
...
- Wald-Wolfowitz (rachas): p=0.0009, umbral BH=0.0125, significativo tras corrección: sí.
- Test de independencia (X_t vs X_t-1): p=0.0287, umbral BH=0.0250, significativo tras corrección: no.
- Kolmogorov-Smirnov (uniformidad): p=0.0459, umbral BH=0.0375, significativo tras corrección: no.
- Chi-cuadrado (uniformidad): p=0.5823, umbral BH=0.0500, significativo tras corrección: no.

## Anexo C: estadística descriptiva adicional (Fase 2)
- Pares/impares: {'par': 0.4927, 'impar': 0.5073}
- Altos/bajos: {'bajo': 0.5026, 'alto': 0.4974}
...
- ADVERTENCIA (entropía condicional): Tabla conjunta de 10000 celdas con solo 6199 transiciones observadas: la mayoría de las celdas tiene 0 o 1 observaciones. La entropía condicional empírica está sesgada hacia abajo en este régimen (subestima la incertidumbre real) y NO debe interpretarse como evidencia de dependencia. Usar el test de independencia (chi-cuadrado sobre bins) para esa conclusión, no este número.
- Distancia de la matriz de transición de Markov a independencia total: 0.5367 (0 = coincide con independencia perfecta). Esta métrica también puede estar inflada por dispersión de la tabla si la muestra es chica; ver el test de independencia (Fase 3) como criterio principal, no este número aislado.
