...
Generado 2026-07-27 12:45:05 · 6120 sorteos analizados (2025-07-21 a 2026-07-25)
...
- Método de Fisher combinando 4 tests independientes: estadístico=11.2607, p-valor combinado=0.1874.

### 2. ¿Existe evidencia estadística de patrones?
- Chi-cuadrado (uniformidad): NO se rechaza H0 (estadístico=96.8627, p=0.5420). 99 grados de libertad.
- Kolmogorov-Smirnov (uniformidad): NO se rechaza H0 (estadístico=0.0161, p=0.0815). Aproximación continua sobre datos discretos (orientativa).
- Anderson-Darling (uniformidad): NO se rechaza H0 (estadístico=1.3420, sin p-valor exacto (valor crítico tabulado)). Comparado contra valor crítico tabulado 2.492 (D'Agostino & Stephens 1986).
- Wald-Wolfowitz (rachas): NO se rechaza H0 (estadístico=-1.5182, p=0.1290). 3001 rachas observadas sobre 6120 valores (esperadas ~3060.4).
- Test de independencia (X_t vs X_t-1): NO se rechaza H0 (estadístico=76.2014, p=0.6301). Tabla 10x10, 81 gl, frecuencia esperada mínima 52.0 (válida).
...
- Espectro de Fourier: potencia relativa del pico dominante = 0.0037. Un proceso aleatorio real tiene un espectro aproximadamente plano (sin picos dominantes); este valor bajo no sugiere periodicidad.
...
- Kolmogorov-Smirnov (uniformidad): p=0.0815, umbral BH=0.0125, significativo tras corrección: no.
- Wald-Wolfowitz (rachas): p=0.1290, umbral BH=0.0250, significativo tras corrección: no.
- Chi-cuadrado (uniformidad): p=0.5420, umbral BH=0.0375, significativo tras corrección: no.
- Test de independencia (X_t vs X_t-1): p=0.6301, umbral BH=0.0500, significativo tras corrección: no.

## Anexo C: estadística descriptiva adicional (Fase 2)
- Pares/impares: {'par': 0.4936, 'impar': 0.5064}
- Altos/bajos: {'bajo': 0.5033, 'alto': 0.4967}
- Entropía de Shannon: 6.6323 bits (máxima posible 6.6439 bits, eficiencia 0.9983). Una eficiencia cercana a 1.0 indica que la distribución observada está cerca de la máxima incertidumbre posible (uniforme).
- ADVERTENCIA (entropía condicional): Tabla conjunta de 10000 celdas con solo 6119 transiciones observadas: la mayoría de las celdas tiene 0 o 1 observaciones. La entropía condicional empírica está sesgada hacia abajo en este régimen (subestima la incertidumbre real) y NO debe interpretarse como evidencia de dependencia. Usar el test de independencia (chi-cuadrado sobre bins) para esa conclusión, no este número.
- Distancia de la matriz de transición de Markov a independencia total: 0.5386 (0 = coincide con independencia perfecta). Esta métrica también puede estar inflada por dispersión de la tabla si la muestra es chica; ver el test de independencia (Fase 3) como criterio principal, no este número aislado.
