...
Generado 2026-07-24 09:40:52 · 6080 sorteos analizados (2025-07-21 a 2026-07-23)
...
- Método de Fisher combinando 4 tests independientes: estadístico=16.9809, p-valor combinado=0.0303.

### 2. ¿Existe evidencia estadística de patrones?
- Chi-cuadrado (uniformidad): NO se rechaza H0 (estadístico=97.6316, p=0.5200). 99 grados de libertad.
- Kolmogorov-Smirnov (uniformidad): NO se rechaza H0 (estadístico=0.0161, p=0.0840). Aproximación continua sobre datos discretos (orientativa).
- Anderson-Darling (uniformidad): NO se rechaza H0 (estadístico=1.3566, sin p-valor exacto (valor crítico tabulado)). Comparado contra valor crítico tabulado 2.492 (D'Agostino & Stephens 1986).
- Wald-Wolfowitz (rachas): NO se rechaza H0 (estadístico=-1.5208, p=0.1283). 2981 rachas observadas sobre 6080 valores (esperadas ~3040.3).
- Test de independencia (X_t vs X_t-1): Se RECHAZA H0 (estadístico=105.2052, p=0.0367). Tabla 10x10, 81 gl, frecuencia esperada mínima 51.4 (válida).
...
- Espectro de Fourier: potencia relativa del pico dominante = 0.0023. Un proceso aleatorio real tiene un espectro aproximadamente plano (sin picos dominantes); este valor bajo no sugiere periodicidad.
...
- Test de independencia (X_t vs X_t-1): p=0.0367, umbral BH=0.0125, significativo tras corrección: no.
- Kolmogorov-Smirnov (uniformidad): p=0.0840, umbral BH=0.0250, significativo tras corrección: no.
- Wald-Wolfowitz (rachas): p=0.1283, umbral BH=0.0375, significativo tras corrección: no.
- Chi-cuadrado (uniformidad): p=0.5200, umbral BH=0.0500, significativo tras corrección: no.

## Anexo C: estadística descriptiva adicional (Fase 2)
- Pares/impares: {'par': 0.4931, 'impar': 0.5069}
- Altos/bajos: {'bajo': 0.5039, 'alto': 0.4961}
- Entropía de Shannon: 6.6321 bits (máxima posible 6.6439 bits, eficiencia 0.9982). Una eficiencia cercana a 1.0 indica que la distribución observada está cerca de la máxima incertidumbre posible (uniforme).
- ADVERTENCIA (entropía condicional): Tabla conjunta de 10000 celdas con solo 6079 transiciones observadas: la mayoría de las celdas tiene 0 o 1 observaciones. La entropía condicional empírica está sesgada hacia abajo en este régimen (subestima la incertidumbre real) y NO debe interpretarse como evidencia de dependencia. Usar el test de independencia (chi-cuadrado sobre bins) para esa conclusión, no este número.
- Distancia de la matriz de transición de Markov a independencia total: 0.5448 (0 = coincide con independencia perfecta). Esta métrica también puede estar inflada por dispersión de la tabla si la muestra es chica; ver el test de independencia (Fase 3) como criterio principal, no este número aislado.
